...
class Virus:
    def __init__(self,name,inputdict):
        ...
        # self.lethality = random.randint(10,10) # WIP
...
def StartSimulation():
    ...
    for i in range(SICKATDAYZERO):
        ...
        PeopleList[i].AddInfection(Virus(Name,NewVirusVar))
        ...
        if len(person.currentInfections) > 0 and person.dead == False:
            ...
            for viralinfection in person.currentInfections:
                ...
                if random.randint(1,100000) == 1:
                    ...
                    person.AddInfection(Virus(Name,NewVirusVar))
                    ...
                    for i in range(10):
                        ...
                        if viralinfection not in Friend.Immunties and viralinfection not in Friend.currentInfections:
                            ...
                            Friend.AddInfection(Virus(Name,NewVirusVar)) #tada
                #-------------------------------------------
...
def PlotData(inputdict):
    ...
    plt.savefig("images/virusfig7")

    print("Plotting Complete")
    plt.show()

def SaveDataAsJson(inputdict):

    with open("jsondata/outdata.txt", "w") as f:
        for person in inputdict.get("PeopleList"):
            for infection in person.currentInfections:
                f.writelines(infection.ReturnData())
                f.writelines("\n")
...
# print(SaveDataAsJson(inputdata))
PlotData(inputdata)
